<template>
  <div class="container w-50">
    ...
    <div class="input-group mb-3">
      ...
      <v-color-picker show-swatches v-model="event.color"></v-color-picker>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      hostEvent(id) {
        organization.getOrganizer(this.id).then((res) => {
          let url = res.data.result.url;
          this.$router.push("/host/" + url + "?auth=org&eventId=" + id);
        });
      },
      add() {
      ...
      getAllevent() {
        event.getAllEventByOrganizer(this.id).then((res) => {
          if (res.data.success) {
